...
AND (t.title LIKE '%$term%' OR t.tags LIKE '%$term%' OR l.category_name LIKE '%$term%' OR sc.category_name LIKE '%$term%' OR u.name LIKE '%$term%') 
GROUP BY t.training_id ORDER BY $order_by LIMIT $limit OFFSET $offset";
...
$rating_data = calculateRating($training_id, $con, $siteprefix);
 ...
 <!-- Category Pagination Section -->
      ...
      <div class="container">
        <nav class="d-flex justify-content-center" aria-label="Page navigation">
          <ul>
                <?php if ($page > 1): ?>
                   <li>
              <a href="?searchterm=<?php echo $term; ?>&page=<?php echo $page - 1; ?>" aria-label="Previous page">
    ...
    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
         <li> <a href="?searchterm=<?php echo $term; ?>&page=<?php echo $i; ?>" class="btn btn-secondary <?php if ($i == $page) echo 'active'; ?>"><?php echo $i; ?></a>  </li>
    <?php endfor; ?>
