...
$query = "SELECT duration, no_of_duration,price,downloads FROM ".$siteprefix."subscription_plans WHERE s = '$plan_id'";
...
if ($plan_result && mysqli_num_rows($plan_result) > 0) {
    ...
    $sql = "INSERT INTO ".$siteprefix."loyalty_purchases (user_id, loyalty_id, amount, start_date, end_date, payment_reference,downloads)
            VALUES ('$user_id', '$plan_id','$amount', '$start_date', '$end_date', '$reference','$downloads')";
    if (mysqli_query($con, $sql)) {
        // Update user subscription
        $update_user = "UPDATE ".$siteprefix."users SET loyalty='$plan_id',downloads='$downloads' WHERE s='$user_id'";

         // Admin commission deduction
         $admin_commission = $amount;
         $sql_insert_commission = "INSERT INTO ".$siteprefix."profits (amount, training_id, order_id, type, date) VALUES ('$admin_commission', '$plan_id', '$plan_id','Subscription Payment','$currentdatetime')";
