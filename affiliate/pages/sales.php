            ...
            <table class="table table-hover">
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        $training_id = $row['product_id'];
                        $sales_count = $row['sales_count'];
                        $total_revenue = $row['total_revenue'];

                        // Get the product name from the training table
                        $product_query = "SELECT title FROM ".$siteprefix."training WHERE training_id = '$training_id'";
                        $product_result = mysqli_query($con, $product_query);
                        $product_data = mysqli_fetch_assoc($product_result);
                        $product_name = $product_data['title'] ?? 'Unknown Product';

                        // Get the product image from the training_images table
                        $image_query = "SELECT picture FROM ".$siteprefix."training_images WHERE training_id = '$training_id' LIMIT 1";
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#transactionsModal_<?php echo $training_id; ?>">View Transactions</button>
                        </td>
                    </tr>
                    <?php
                        // Generate modal content separately
                        $modals .= '
                        <div class="modal fade" id="transactionsModal_'.$training_id.'" tabindex="-1" aria-labelledby="transactionsModalLabel_'.$training_id.'" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="transactionsModalLabel_'.$training_id.'">Transactions for '.htmlspecialchars($product_name).'</h5>
                        ...
                        $transactions_query = "
                            ...
                            WHERE 
                                oi.training_id = '$training_id' AND oi.affiliate_id = '$affliate' AND o.status = 'paid'
                        ";
                        ...
                        while ($transaction = mysqli_fetch_assoc($transactions_result)) {
                            ...
                            $profile_picture = $transaction['profile_photo'] ?? 'default-profile.jpg';
